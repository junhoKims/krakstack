...
describe('withComma', () => {
  ...
  test('인자에 숫자가 아닌 문자가 포함되어 있으면 에러 리턴', () => {
    expect(() => withComma('123456789원')).toThrowError(/\숫자인 문자열만 허용합니다/);
    expect(() => withComma('가나다라바사바')).toThrowError(/\숫자인 문자열만 허용합니다/);
  });

  test('인자 NaN이라면 에러 리턴', () => {
    expect(() => withComma(NaN)).toThrowError(/NaN은 허용하지 않습니다/);
  });
});
